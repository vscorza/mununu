...
| `motivating_examples/` | Three additional real C files validating extractor claims end-to-end via real clang (CLAUDE.md claims-integrity rule #10). `example_2_typecast_register_access.c` exercises `*(volatile uint32_t *)0xADDR` (the Zephyr/FreeRTOS BSP convention). `example_5_isr_with_main_thread.c` exercises `@mununu_isr` + asynchronous composition emission. `example_6_multi_entry_driver.c` exercises `--driver-mode` dispatch over three entry points (`uart_init`, `uart_send`, `uart_recv`). Run `motivating_examples/validate-motivating-examples.sh` for a byte-deterministic transcript. **Example 4 (helper functions with pointer parameters) is deliberately absent** — phase L5.5 (pointer-parameter alias tracking) is queued; the gap is documented at `docs/design/c-extraction-correctness-scope.md`. |
