...
use mununu::abstraction::constraint::Constraint;
use mununu::abstraction::domains::{BoolDomain, IntDomain};
use mununu::abstraction::evaluator::{EvaluationError, evaluate_expr, evaluate_guard};
use mununu::abstraction::expression::{Expr, GuardExpr, GuardResult};
use mununu::abstraction::state::AbstractState;
use mununu::abstraction::value::AbstractValue;
use mununu::guard::ComparisonOp;
...
fn test_constraint_creation() {
    ...
    assert!(matches!(
        constraint.kind,
        mununu::abstraction::constraint::ConstraintKind::VarConst { .. }
    ));

    let constraint = Constraint::var_var("x".to_string(), ComparisonOp::Lt, "y".to_string());
    assert!(matches!(
        constraint.kind,
        mununu::abstraction::constraint::ConstraintKind::VarVar { .. }
    ));
}
