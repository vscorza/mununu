...
The web UI compositional extraction step is filed as a follow-up; once it lands, every step covered here will also be reachable through the workflow stepper.

## Out of scope

- **Pattern-based auto-detection** of `asyncio.gather` / `multiprocessing.Process` / `Promise.all` is Phase B. The extractor today requires you to declare `instances` manually; it doesn't infer them from source.
