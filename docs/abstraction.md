...
**For the BTOR2 predicate cube specifically**, the transfer above holds for the fragment the cube path actually evaluates — `Control::All`, bare (label-agnostic), unbounded modalities — and rests on two premises now mechanised in CI: the lift is a sound may/must KMTS (`may ⊇ concrete`, `must ⊆ concrete`) and the evaluator computes the §4.3 modal semantics. Out-of-fragment modalities (controllability `ctrl=…`, bounded `steps=k`, label-specific on a non-cube label) are **flagged with a soundness warning, not silently decided** — controllability in particular awaits the per-player game audit (R.6.8) before a *definite* verdict is sound. Full write-up: [`predicate-abstraction-recipe.md`](design/predicate-abstraction-recipe.md) §4.9.
