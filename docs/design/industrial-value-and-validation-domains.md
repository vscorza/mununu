...
**V.3** ships a small in-order pipeline + branch predictor + cache fixture, self-composed with a public/secret input split, verifying speculative non-interference for a hand-crafted contract. This is research-grade; the goal is to demonstrate the modeling pattern rather than to ship a production-ready Spectre checker. Fixture: [`examples/verify/v3_specsafe_pipeline/`](../../examples/verify/v3_specsafe_pipeline/) (ships with V.3).
