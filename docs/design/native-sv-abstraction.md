...
| **R.0c — Pipeline-faithfulness regression harness** | New CLI `mununu sv compare-pipelines <fixture>` that runs both native and BTOR2-per-module paths, diffs verdicts + intermediate-shape stats. Includes SVA-elision gate writing `sva_elision_gate.json`. | 1 wk | `kmts_pipeline_baseline.json` for every fixture; CI job runs polarity comparisons. Gates S.0–S.2. |
